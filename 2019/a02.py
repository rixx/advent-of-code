...
def run(program):
    ...
    memory = {index: int(number) for index, number in enumerate(program)}
    instruction_pointer = 0
    while memory[instruction_pointer] != 99:
        opcode = memory[instruction_pointer]   # Default 4-value instruction: opcode + 3 parameters
        if opcode == 1:
