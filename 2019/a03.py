...
def draw_lines(code): 
    ...
    for line in code.strip().split('\n'): 
        ...
        for section in line.strip().split(","): 
            ...
            xdiff, ydiff = direction_dict[direction] 
...
min(abs(x) + abs(y) for (x, y), cable_count in r.items() if cable_count > 1)
...
"""
def intersect_times(code, layout): 
    direction_dict = {'R': (1, 0), 'L': (-1, 0), 'U': (0, 1), 'D': (0, -1)} 
    intersections = collections.defaultdict(dict)
    for line in code.strip().split('\n'): 
        x = 0 
        y = 0 
        steps = 0
        for line_count, section in enumerate(line.strip().split(",")):
            direction = section[0] 
            distance = int(section[1:]) 
            xdiff, ydiff = direction_dict[direction]
            for _ in range(distance): 
                ...
                x += xdiff
                y += ydiff
                if layout[(x, y)] > 1 and not line_count in intersections[(x, y)]:
                    intersections[(x, y)][line_count] = steps
    return intersections


def get_min_intersection(code):
    layout = draw_lines(code)
    intersections = intersect_times(code, layout)
    return min(sum(section.values()) for section in intersections.values() if len(section) > 1)
