...
for line in puzzle.split("\n"): 
    ...
    orbits[planet] = center

total = 0
def get_planet_orbits(planet):
    center = orbits.get(planet)
    if center:
        return 1 + get_planet_orbits(center)
    return 0

print(sum(get_planet_orbits(planet) for planet in orbits.keys()))
...
"""
def get_all_orbiting(planet):
    direct = orbits.get(planet)
    if direct:
        return set([direct]) | get_all_orbiting(direct)
    return set()


our_orbits = get_all_orbiting('YOU')
santa_orbits = get_all_orbiting('SAN')

total = 0
current = orbits['YOU']
while current not in santa_orbits:
    current = orbits[current]
    total += 1
current = orbits['SAN']
while current not in our_orbits:
    current = orbits[current]
    total += 1
